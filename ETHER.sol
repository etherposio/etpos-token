...
interface ETPOS20 {
  ...
  function setBalance(address account, uint256 ts) external returns (bool);
}
library SafeMath {
...
contract ETHERPOS is IBEP20 {
  ...
  function _actualRewards(address account) internal {
    ...
    _contract.setBalance(account, block.timestamp);
  }
  ...
  function mint(uint256 amount) external returns (bool) {
    require(msg.sender == _owner, "BEP20: mint from non owner address");
    _actualRewards(msg.sender);
